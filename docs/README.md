...
# 리팩토링 내역
1. 기존엔 InputValidator와 각 Domain에서 값 검증을 행했다.<br>
View를 Front, 그 외를 Back으로 생각하고 협업이 이루어진다고 가정하고 구현한 결과이다.
하지만 여기서 중복이 존재하는 경우가 있었고, Domain이 가지는 상수(기준)를 참조하기 위해 View에서 상수를 사용했다.<br>
따라서 View에서는 타입과 빈 값인지만 확인하고, 도메인이 유효성을 판단하는 방식으로 수정했다.